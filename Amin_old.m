classdef Amin < handle
    ...
    methods
        function obj=Amin(V,dt)
            ...
            % SL dynamics
            obj.d_SL        = @(SL, IntegF) (obj.contrflag.*((IntegF + obj.viscosity.*(obj.SL_rest - SL))./obj.mass)) .* (obj.SL_collagen <= obj.preload_SL)+  0 .* (obj.SL_collagen > obj.preload_SL);
            %F_passive = passiveForces(SL,passive);
            %dIntegF = (-F_passive-Fnorm+Afterload);
            % Rate of change of the average distortions
            obj.d_x_XBpreR  = @(SL, x_XBpreR, x_XBpostR, IntegF) obj.d_SL(SL, IntegF)./2 + (obj.Psi./obj.dutyPreR(x_XBpreR, x_XBpostR)).*(-obj.ap1.*x_XBpreR) + obj.am2.*(x_XBpostR-obj.x0-x_XBpreR);
            obj.d_x_XBpostR = @(SL, x_XBpreR, x_XBpostR, IntegF) obj.d_SL(SL, IntegF)./2 + (obj.Psi./obj.dutyPostR(x_XBpreR, x_XBpostR)).*(obj.ap2(x_XBpreR).*(x_XBpreR + obj.x0 - x_XBpostR));
            ...
            obj.dsovr_ze    = @(SL) -dSL(SL)./2.*heav(obj.L_thick-SL);
            obj.dsovr_cle   = @(SL) -dSL(SL)./2.*heav((2.*obj.L_thin-SL)-obj.L_hbare);
        ...
        function Iion=solveTimestep(obj,Vold,i)
            ...
            d_SL_eval       = obj.d_SL(obj.SL_o, obj.IntegF_o);
